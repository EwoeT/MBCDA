...
#         # print("tryy",config.vocab_size, 50265, embed_dim)
...
class BartForSequenceClassification(BartPreTrainedModel):
    ...
    #Mean Pooling - https://huggingface.co/sentence-transformers/all-mpnet-base-v2
    def mean_pooling(self, model_output):
        token_embeddings = model_output #First element of model_output contains all token embeddings
        # input_mask_expanded = attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float()
        return torch.sum(token_embeddings, 1)
    ...
    def forward(
        ...
        # print(outputs.shape)
        hidden_states = outputs  # last hidden state

        # Perform pooling
        sentence_embeddings = self.mean_pooling(hidden_states)

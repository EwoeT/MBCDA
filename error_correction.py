...
class gen_text():
    ...
    def generate(self, text_list):
        ...
        with torch.no_grad(): 
            fake_inputs = self.tokenizer.batch_encode_plus(text_list, truncation=True, max_length=200, padding=True, return_tensors="pt")["input_ids"].to("cuda")
            ...
            inputs = self.bart_tokenizer.batch_encode_plus(output_texts1, truncation=True, max_length=200, padding=True, return_tensors="pt").to("cuda")
            summary_ids = self.bart_model.generate(input_ids = inputs["input_ids"], num_beams=1, do_sample=False, min_length=0, max_length=200)
            # summary_ids = self.normalizer_model.generate(summary_ids, num_beams=1, do_sample=False, min_length=0, max_length=200)
